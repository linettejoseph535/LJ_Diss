...
# ================= FIND LATEST EFFICIENTNET MODEL =================
...
if not os.path.exists(model_dir):  # checks if the folder exists
    raise FileNotFoundError(f"Model directory not found: {model_dir}")  # stops if missing

model_files = [f for f in os.listdir(model_dir) if "efficientnet" in f.lower() and f.endswith(".keras")]  
# gets all .keras files that contain "efficientnet" in the name

if not model_files:  # checks if no EfficientNet models were found
    raise FileNotFoundError("No EfficientNet .keras model files found in outputs/models")  # stops the script

model_files.sort()  # sorts the model files alphabetically
latest_model = model_files[-1]  # Picks the newest model

model_path = os.path.join(model_dir, latest_model)  # full path to the model file
print(f" Loading EfficientNetB0 model: {model_path}")  # shows which model is being loaded

# ================= LOAD MODEL =================
model = tf.keras.models.load_model(model_path)  # loads the EfficientNetB0 model
...
# ================= PREPROCESS =================
def preprocess(img):  #  function - prepare an image for EfficientNet
    ...
    img = tf.keras.applications.efficientnet.preprocess_input(img)  # applies EfficientNet preprocessing
...
def predict(image_path):  #function- predict a single image
    ...
    if img is None:  # checks if loading failed
        raise ValueError(f"Image not found: {image_path}")  # stops with an error

    img = cv2.cvtColor(img, cv2.COLOR_BGR2RGB)  # converts (blue green red)BGR to RGB (red green blue)
    x = preprocess(img)  # preprocesses the image

    prob = model.predict(x, verbose=0)[0][0]  # gets the model’s probability score
    label = int(prob > 0.5)  # converts probability into 0 or 1

    return label, float(prob)  # returns label and probability
...
def evaluate_test_set():  # function - test the entire dataset
    ...
    if not os.path.exists(test_dir):  # checks if test folder exists
    ...
    for label_name in ["fake2", "real2"]:  # loops through both classes
        ...
        for img_name in os.listdir(class_dir):  # loops through each image
            img_path = os.path.join(class_dir, img_name)  # full path to the image

            pred_label, prob = predict(img_path)  #predicts the image

            # Ground truth: fake2=0, real2=1  # explains true label mapping
            true_label = 0 if label_name == "fake2" else 1  #converts folder name to label

            if pred_label == true_label:  # cecks if prediction is correct
            ...
            # prints filename, predicted label, true label, and probability

    accuracy = correct / total if total > 0 else 0  # calculates accuracy
