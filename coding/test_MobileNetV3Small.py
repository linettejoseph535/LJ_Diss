...
CONFIG = {
    ...
    "plots_dir": r"C:\LJ_Diss\outputs\plots\mobilenet",  #  graphs will be saved here
}
...
# ================= LOAD LATEST MOBILENET MODEL =================
def get_latest_mobilenet(model_dir):  #this function - looks for the most recently saved MobileNet model
    models = glob.glob(os.path.join(model_dir, "*mobilenet*.keras"))  # searches the folder for files with "mobilenet" in the name
    if not models:  #checks if the list is empty
        raise ValueError("No MobileNet models found")  # Stops the program if no models exist
...
def load_dataset(path):  # function- loads images from a folder
    ...
    def preprocess(x, y):  # function to prepare each image
        x = tf.keras.applications.mobilenet_v3.preprocess_input(x)  #aplies MobileNetV3’s required preprocessing
...
def plot_cm(y_true, y_pred, classes, save_path): #function - draw and save a confusion matrix
    ...
    sns.heatmap(cm, annot=True, fmt="d", cmap="Blues",  # draws the confusion matrix as a coloured grid
    ...
    plt.title("MobileNetV3 Confusion Matrix")  #title of the plot
...
if __name__ == "__main__":  # ensures this code only runs when the file is executed directly
    ...
    model_path = get_latest_mobilenet(CONFIG["model_dir"])  # finds the newest MobileNet model file
    print("Loading model:", model_path)  # prints the path of the model being loaded

    model = tf.keras.models.load_model(model_path)  # loads the trained MobileNetV3Small model
    ...
    # ================= CONFUSION MATRIX ================= 
    cm_path = os.path.join(CONFIG["plots_dir"], "mobilenet_confusion_matrix.png")  # creates the full file path for the confusion matrix image
    ...
    print("\n MobileNetV3Small testing complete!") 
