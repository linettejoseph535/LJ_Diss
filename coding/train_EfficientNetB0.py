...
CONFIG = {
    ...
    "img_size": (224, 224), # images resized - for EfficientNetB0
    ...
    "gradcam_dir": r"C:\LJ_Diss\outputs\gradcam_outputs\efficientnet"  # Gradcam images saved in this path
}
...
# TensorFlow automatically assigns labels based on the folder names
    ...
    def preprocess(x, y):
        x = tf.keras.applications.efficientnet.preprocess_input(x) # applies EfficientNetB0 preprocessing
        y = tf.cast(y, tf.float32) # ensures labels are float values eg;32
    ...
    if augment:
        ...
        # all under different conditions and prevents it from memorising the training data
            ...
            tf.keras.layers.RandomRotation(0.08), # This rotates the image slightly in either direction - helps the model handle faces that are tilted or photographed at a slight angle
            tf.keras.layers.RandomZoom(0.08), # This zooms in or out by a small amount - prepares the model for faces that appear at different distances from the camera
...
# ================= MODEL =================
def build_model():
#Builds a EfficientNetB0 model with a custom classifier and a named layer for Gradcam
    ...
    x = tf.keras.layers.Dense(128, activation="relu")(x)  # fully connected layer for learning image patterns
    x = tf.keras.layers.Dropout(0.4)(x) #reduces overfitting by randomly dropping units
...
#loads and prepares a single image for Gradcam
    ...
    arr = tf.keras.applications.efficientnet.preprocess_input(arr) #prepares the image in the format EfficientNetB0 was trained on
    return arr
...
# saves image showing three panels: the original image,the Gradcam heatmap, and the Gradcam++ heatmap.  
    ...
    combined = np.hstack([img, cam_hm, campp_hm])
...
if __name__ == "__main__":
    ...
    for layer in base.layers[:-30]:  #keeps earlier layers frozen to avoid overfitting.
    ...
    # SAVE MODEL
    model_path = os.path.join(CONFIG["model_dir"], f"efficientnetb0_50ep_{timestamp}.keras")
    ...
    print("\nGradcam complete for EfficientNetB0.")
